...
class Nonin9560SPPDataFormat2 extends ICE_Periodic_Exchange {
  override val access : Option[ICE_Security_Access_Read] = None
  override val separation_interval : NatRange = new NatRange {
    override val min : Nat = 333
    override val max : Nat = 333
...
class Nonin9560SPPDataFormat7 extends ICE_Periodic_Exchange {
  override val access : Option[ICE_Security_Access_Read] = None
  override val separation_interval : NatRange = new NatRange {
    override val min : Nat = 333
    override val max : Nat = 333
...
class Nonin9560SPPDataFormat8 extends ICE_Periodic_Exchange {
  override val access : Option[ICE_Security_Access_Read] = None
  override val separation_interval : NatRange = new NatRange {
    override val min : Nat = 1000
    override val max : Nat = 1000
  }
}

final class Nonin9560PulseOx extends ICE_MDS {
  ...
  override val manufacturerModel : ICE_ManufacturerModel = new ICE_ManufacturerModel {
    override val MDC_ATTR_ID_MODEL : IEEE11073_SystemModel = new IEEE11073_SystemModel {
      ...
      override val model_number : String = "Onyx II Model 9560"
    }
      ...
      override val channels : Map[String, ICE_Channel] = Map(
        "spo2" -> new ICE_SpO2_Channel {
          ...
          override val metrics : Map[String, ICE_Metric] = Map(
              /*
               * Three options for representing waveforms:
               * 1) Different numerics on the same channel
               * 2) Same numeric, but waveforms are sequences while normal readings aren't
               * 3) Same numeric, normal readings are singleton sequences
               */
        ...
        "pulserate" -> new ICE_PulseRate_Channel {
          ...
          override val metrics : Map[String, ICE_Metric] = Map(
            "pulserate_num" -> new ICE_PulseRate_Numeric {
              override val range : FloatRange = new FloatRange {
                override val min : Float = 18.0
                override val max : Float = 321.0
              }
              override val exchanges : Map[String, ICE_Data_Exchange] = Map(
                "get" -> new NoninGetExchange {},
                "periodic" -> new NoninPeriodicExchange {}
