...
public class WellformednessTest {
  ...
  public void testBasicTypeWellformedness() throws Exception {
    final Model m = loadModel("dms.test", "edu.ksu.cis.santos.mdcf.dms.example");
    BasicTypeWellFormednessChecker checker = new BasicTypeWellFormednessChecker(m);
    checker.init();
  ...
  public void testBasicTypeWellformednessCyclic() throws Exception {
    Model m = model(list(basicType("A", list(namedType("B"))), basicType("B", list(namedType("A")))));
    String mXml = toXml(m);
    System.out.println(mXml);
  ...
  Model loadModel(final String name, final String... packageNames) throws Exception {
