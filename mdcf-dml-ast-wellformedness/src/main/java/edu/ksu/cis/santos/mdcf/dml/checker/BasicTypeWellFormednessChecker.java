...
public class BasicTypeWellFormednessChecker{
  ...
  public void init(){
    ...
    System.out.println("Inheritance Relation List: " + v.inheritanceRelations);
    ...
    System.out.println("Complete Inheritance Relation List: " + this.completeInheritanceRelations);
    
  }

  public boolean checkBasicTypeWellformedness_NonBasicType(){
    //For each basic type, each of its super types is either another basic type or dms.BasicType.
      ...
      if(this.completeInheritanceRelations.get(key) != null){
        for(String type : this.completeInheritanceRelations.get(key)){
          if(!this.basicTypeList.contains(type)){
            //the type is not a basic type or not inheriting from basic type.
            System.err.println(key + " inherits from a non-basic type: " + type);
            return false;
          }
        }
      }
  ...
  public boolean checkBasicTypeWellformedness_CyclicInheritance(){
    ...
    for(String key: this.basicTypeList){
      if(this.completeInheritanceRelations.get(key) != null){
        ArrayList<String> fullList = this.completeInheritanceRelations.get(key);
        if(fullList.contains(key)){
          //If the complete inheritance list contains the root type, it is cyclic.
  ...
  private void buildInheritanceHierarchy(String original, String current, ArrayList<String> fullList){
    ArrayList<String> hierarchyList = this.inheritanceRelations.get(current);
    if(hierarchyList == null || hierarchyList.isEmpty()){
      ...
      for(String parent: hierarchyList){
        ...
        //We won't notice anything here but stop building the hierarchy.
        if(parent.equals(original)) return;
